"""Resume vs. job description fit checker: FastAPI + Groq (bring your own API key).

Mirrors the n8n workflow:
  form submission -> extract JD text + extract resume text -> merge -> LLM -> result

The user types a Groq model name and API key into the web page. They are sent with each request
and used only for that request: the server never stores or logs them and never uses a key of its
own, so token cost goes to whoever typed their key in.
...
Optional environment variables (a local .env file works too, see .env.example):
  GROQ_MODEL      model name prefilled in the page (users can change it)
  GROQ_BASE_URL   override the Groq API base URL
...
class GroqClient:
    """Groq's OpenAI-compatible API, using the model name and API key typed into the page."""

    def __init__(self, model: str, api_key: str) -> None:
        self.model = model
        self.finish_reason = ""  # why the last call stopped ("stop", "length", ...), for diagnostics
        self._api_key = api_key

    @classmethod
    def from_request(cls, model: str, api_key: str) -> "GroqClient":
        """Validate what the page sent and build a client for this one request."""
        model = (model or "").strip() or DEFAULT_MODEL
        if not model:
            raise LLMError("Enter a Groq model name.")
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(120, connect=10)) as client:
                resp = await client.post(
                    f"{GROQ_BASE_URL}/chat/completions", headers=self._headers, json=payload
                )
    ...
    async def json_completion(self, prompt: str) -> str:
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(300, connect=10)) as client:
                async with client.stream(
                    "POST", f"{GROQ_BASE_URL}/chat/completions", headers=self._headers, json=payload
                ) as resp:
                    if resp.status_code != 200:
...
async def extract_required_skills(llm: GroqClient, jd_text: str) -> list[str]:
    ...
    content = await llm.json_completion(prompt)
    try:
...
async def stream_report(llm: GroqClient, prompt: str, rating_block: str):
    ...
    try:
        ...
        if chars == 0:
            ...
            log.warning("empty stream from %s (finish=%s), retrying without streaming",
                        llm.model, llm.finish_reason)
            text = strip_think(await llm.text_completion(prompt)).strip()
            if not text:
                raise LLMError(
                    "The model returned no answer text "
                    f"(finish reason: {llm.finish_reason or 'unknown'}). "
        ...
        yield f"\n[[ERROR]] {exc}"
        return
    log.info("report done: model=%s chars=%d finish=%s", llm.model, chars, llm.finish_reason)
    yield rating_block
...
async def match(
    ...
    try:
        llm = GroqClient.from_request(model, api_key)
    except LLMError as exc:
...
@app.get("/api/config")
async def config():
    """Default for the model name box. Never includes any key."""
